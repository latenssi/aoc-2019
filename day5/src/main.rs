...
struct InstructionSet {
    ...
    ops: HashMap<Operation, fn(&mut Memory, &mut Register, &Instruction) -> Option<isize>>,
}
...
impl InstructionSet {
    ...
    fn execute(&self, m: &mut Memory, r: &mut Register, i: &Instruction) -> Option<isize> {
        match self.ops.get(&i.operation) {
            Some(f) => f(m, r, i),
